    ...
    match vtype:
        ...
        case 'Integer':
            try:
                ...
                initial_value = "Incorrect value: " + \
                    window['Initial ValueEdit'].window_text()
        case _:
            raise Exception(f"Not implement initial value for {vtype=}")
    return Variable(name=name, type=vtype, is_constant=is_constant, initial_value=initial_value)
...
def parse_nodes(app, node: _treeview_element, cache: Cache, prev_path: None | NodePath = None,  is_first: bool = False) -> Node:
    node_text = node.text()
    node.select()
    ...
    n.childrens = [parse_nodes(app, child, prev_path=path, cache=cache) for child in childrens]
    return n
